...
#define CAPACITY 10
#define VIPSTR(vip) ((vip==1) ? "  vip  " : ((vip==2)? "special": "not vip"))
...
void enter_special_client(int id){
	...
	special_waiting ++;
	printf("special: %d, esperando\n", id);
	while(current_ocupancy >= CAPACITY || n_not_special>0 || turno != turno_actual_special){
	...
	pthread_cond_broadcast(&esperar_turno); // esto no entiendo porq
...
void enter_vip_client(int id)
{
    ...
    while (current_ocupancy >= CAPACITY || turno != turno_actual_vip ||
		current_ocupancy-n_not_special>0 || special_waiting>0)
    {
        pthread_cond_wait(&esperar_turno, &mutex);
    } 
...
void enter_normal_client(int id)
{
    ...
    while (current_ocupancy >= CAPACITY || turno != turno_actual ||
		vip_waiting > 0 || current_ocupancy-n_not_special>0 || special_waiting>0) {
        pthread_cond_wait(&esperar_turno, &mutex);
    }
    turno_actual++;
    current_ocupancy++;
